...
.controller('templatesCtrl', function($scope, $ionicConfig) {
//alert('hey');

})
.controller('doctorScreensCtrl', function($scope, $ionicConfig) {
	$scope.settingsList = [
	    { text: "Online", checked: true },
	    { text: "Offline", checked: false }
	  ];
...
.controller('patientrequestCtrl', function($scope, $ionicConfig) {
//alert('hello');
$scope.toggle = true;
		...
		var loginpin=$scope.user.pin;
			...
			data.loginprocess(loginphone,loginpin).then(function(response) {
				...
				//	$state.go('auth.doctorscreens');
					$state.go('templates.doctorscreens');
...
.controller('FeedsCategoriesCtrl', function($scope, $http) {
	...
	$http.get('patientScreens.json').success(function(response) {
